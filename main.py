...
# tp: temp, h: hum, lt: latitude, lg: longitude, t: time
uploadData = {"tp":0, "h":0,"lt":0, "lg":0, "t":0}
while not dataReady:
    
    result = th.read()
    if result.is_valid() and not tempReady:
        pycom.rgbled(0x001000) # green
        tempReady = True
        print("Temperature: %d C" % result.temperature)
        print("Humidity: %d %%" % result.humidity)
        uploadData["tp"] = result.temperature
        uploadData["h"] = result.humidity
    if uart.any():
        data = uart.read(1)
        # print("gps data -> {}".format(data))
        try:
            stat = my_gps.update(data.decode("utf-8")) # Note the conversion to to chr, UART outputs ints normally
            if stat:
                print("stat:{}".format(stat))
                print(my_gps.latitude,my_gps.longitude,my_gps.date_string(),my_gps.timestamp)
                if my_gps.latitude and my_gps.longitude:
                    if len(my_gps.latitude) == 3 and len(my_gps.longitude) == 3 and len(my_gps.timestamp) == 3:
                        
                        lat = my_gps.latitude[0] + my_gps.latitude[1] / 60
                        uploadData["lt"] = lat if lat != 0 else 0
                        if my_gps.latitude[2] == "S":
                            uploadData["lt"] = -1 * lat if lat != 0 else 0
                        
                        long = my_gps.longitude[0] + my_gps.longitude[1] / 60
                        uploadData["lg"] = long if long != 0 else 0
                        if my_gps.longitude[2] == "W":
                            uploadData["lg"] = -1 * long if long != 0 else 0
                        
                        # uploadData["alt"] = my_gps.altitude if my_gps.altitude != 0 else 0
                        
                        if lat == 0 and long == 0 and my_gps.date_string():
                            pycom.rgbled(0x100000) # red
                            print("GPS Data Not Ready")
                            time.sleep(1)
                            pycom.rgbled(0x011100) # red
                            time.sleep(10)
                        else:
                            
                            gpsReady = True
                            uploadData["t"] = my_gps.date_string() + " {}:{}:{}".format(my_gps.timestamp[0],my_gps.timestamp[1],my_gps.timestamp[2])
                            print("GPS Data Ready")
